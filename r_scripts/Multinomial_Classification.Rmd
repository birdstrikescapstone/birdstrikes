...
# Create RatiW column by taking the mean of ratio grouped over year and week
# ratio_week <- class.data %>% group_by(YEAR, WEEK) %>% summarise(RATIOW = mean(RATIO))

# Merge the RatioW column back to the main dataframe
# class.data <- inner_join(class.data,ratio_week, by=c("YEAR"="YEAR", "WEEK"="WEEK"))
...
boruta.df <- boruta.df %>% filter(boruta.df$decision == "Confirmed" & boruta.df$meanImp > 8)
