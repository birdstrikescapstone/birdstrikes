...
```{r Create Model Data, echo=FALSE, message=FALSE, warning=FALSE}


########### Data Pre-Processing  ###########
# Read dataset from RDS and filter data for Denver International Airport
reg.data <- readRDS("../data/AIRFIELDS_MASTER_V2.RDS")

# Rename Airfield and filter data by airfield
reg.data <- reg.data %>% rename(AIRFIELD = `AIRPORT ID`)

reg.data <- reg.data %>% filter(reg.data$AIRFIELD == airportName)

# Reorder data frame by Date 
reg.data <- reg.data[order(reg.data$DATE), ]

# Remove outliers or missing data
reg.data <- reg.data[!(reg.data$SLP == 9999.9 |
                         reg.data$MXSPD == 999.9 |
                         reg.data$PRCP == 99.99),]

# Create lag variables for precipitation and Temperature
...
# Remove unused variables in the regression Analysis
reg.data <-
  subset(
    reg.data,
    select = -c(
...
```{r Test Train Splits, echo=FALSE, message=FALSE, warning=FALSE}

# Create the training and test datasets
set.seed(500)

# Step 1: Get row numbers for the training data and perform a 70-30 split
trainRowNumbers <-
...
```{r Regression Formula, echo=FALSE, message=FALSE, warning=FALSE}
# Regression Formula for use in all the models
reg.formula <-
  STRIKECOUNT ~  FLIGHTCOUNT + TEMP + PRCP + SEASON + SLP + MXSPD + PRCP_LAG_1 + TEMP_LAG_1 + BIRDCOUNT + FOG + RAIN_DRIZZLE

```


```{r Model Metrics Data Frame, echo=FALSE, message=FALSE, warning=FALSE}
# Create an Empty Dataframe for capturing the metrics of each model execution
model.metrics <-
  data.frame(
...
```{r Function Check Predictions, echo=FALSE, message=FALSE, warning=FALSE}
########################################################################
# Check Regression Model outcomes True positives and False positives

# Input:  Validation data with all the Raw predictions
#         Model Method Learner

# Output: Count of Positives and Negatives
########################################################################   
...
```{r Function Save Model Metrics, echo=FALSE, message=FALSE, warning=FALSE}

########################################################################
# Save Regression Model Metrics like RMSE, MAE, NMSE, NMAE

# Input:  Model Predictions
#         Test data
#         Model Learner Name

# Output: List with Model method learner and the metrics for each model
########################################################################          
...
saveModelMetrics <-
  function(model.predictions,
           ...
           model.name) {
    # rmse
    rmse <- round(sqrt(mean((data$STRIKECOUNT - model.predictions) ^ 2)), 3)
    
    # Mean Absolute Error (MAE) - average absolute deviation between the predictions and the true values
    mae <- round(mean(abs(data$STRIKECOUNT - model.predictions)), 3)
    
    # Normalized Mean Squared Error (NMSE)
    nmse <- round(sum((data$STRIKECOUNT - model.predictions) ^ 2) / sum((
      data$STRIKECOUNT - mean(data$STRIKECOUNT)) ^ 2), 3)
    
    #Normalized Mean Absolute Error (NMAE)
    nmae <- round(sum(abs(data$STRIKECOUNT - model.predictions)) / sum(abs(
      data$STRIKECOUNT - mean(data$STRIKECOUNT))), 3)
    
    # combine all the above validation metrics
    model.summary <- c(model.name,
                       rmse,
                       mae,
                       nmse,
                       nmae)
    
    # Return Model Metrics to the caller
...
```{r Validate Model Function, echo=FALSE, message=FALSE, warning=FALSE}

########################################################################
# Validate Regression Model Metrics - Uses all the above functions to 
...
# Output: Dataframe with predictions for the specific model executed
########################################################################   
...
validate.models <- function(model, model.predictions, data, model.name) {
  ...
  # plot the regression residual and predictino plots
  plot(
    validate$predicted,
    col = "red",
    type = "l",
...
model.lm <- train(
  ...
  trControl = trainControl(
    method = "cv",
    number = 5,
    index = createFolds(as.factor(train.data$STRIKECOUNT), 5),
    allowParallel = TRUE
  ),
  tuneLength = 5
)

# Print model summary on console 
summary(model.lm)
model.lm
...
# Save Model metrics in dataframe
model.metrics[nrow(model.metrics) + 1,] <-
  saveModelMetrics(model.predictions, test.data, model.name)

# Validate and plot model outputs
check.test.data <-
...
pred.glm<-pred.check(check.test.data,model.name)
...
```{r Negative Binomial, echo=FALSE, message=FALSE, warning=FALSE}
...
#Creates a prediction table that checks how many are false where we failed to predict a strike 
pred.nb<-pred.check(check.test.data,model.name)
...
```{r Random Forest Regressor, echo=FALSE, message=FALSE, warning=FALSE}
...
#Creates a prediction table that checks how many are false where we failed to predict a strike
pred.rf <- pred.check(check.test.data, model.name)
...
```{r SVM Regression, echo=FALSE, message=FALSE, warning=FALSE}
...
#Creates a prediction table that checks how many are false where we failed to predict a strike
pred.svm <- pred.check(check.test.data, model.name)
...
```{r XGBoost Regressor, echo=FALSE, message=FALSE, warning=FALSE}

# XGBoost Regressor
model.name <- "XGBoost Regressor"
rm(check.test.data)

# Copy reg.data to a new dataframe
# We need to perform the below steps as XGBoost does not allow text values 
# and does not convert text to factors automatically
...
# Create dummies
dummies <- dummyVars(~ SEASON, data = xgb.data)
...
# predict on test data
model.predictions <- predict(model.xgb, test.data)
...
# Creates a prediction table that checks how many are false where we failed to predict a strike
pred.xgb <- pred.check(check.test.data, model.name)
...
##### H2o AutoML for tune Parameters
