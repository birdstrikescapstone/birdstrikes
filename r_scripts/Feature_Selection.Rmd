...
4.	Weather –We queried weather data from the National Oceanic and Atmospheric Administration by the US Department of Commerce in order to enhance our master dataset with strike conditions. Weather variables, such as sea level pressure, visibility, temperature, precipitation, and wind speed, allow us to understand the correlation or effect of environmental conditions on bird strikes.
...
Before we start modeling we aggregate the data by day and remove all the insignificant from the original dataset. Also, split the date column into day, month, year and week of year and bind the columns to the original dataset.
...
########### Data Pre-Processing  ###########
# Read dataset from RDS and filter data for just Denver International Airport
model.data.day <- readRDS("../data/AIRFIELDS_MASTER_V2.RDS")

model.data.day <- model.data.day %>% rename(AIRFIELD = `AIRPORT ID`)

model.data.day <- model.data.day %>% filter(model.data.day$AIRFIELD == airportName)
...
corr.data <- model.data.day %>% dplyr::select(BIRDCOUNT,
                                   # FLIGHTCOUNT,
                                   TEMP,
                                   # DEWP,
                                   SLP,
...
Based on the above plot, Temperature and Dew point are positively and strongly correlated with each other. Max wind speed and wind speed are also postively strongly correlated with each other. Temperature and visibility are also correlated to with a correlation coefficient of 0.38 and slight positive correlation
...
```{r Corr Plot with revised variable list}
...
```{r Boruta Multinomial}
# Execute Boruta for Multinomial classification
var.boruta <-
  Boruta(
    as.factor(RISK) ~ BIRDCOUNT + TEMP + SLP + PRCP + SEASON + MXSPD + PRCP + FOG + RAIN_DRIZZLE + HAIL + THUNDER,
    getImp=getImpExtraZ,
    data = model.data.day,
    doTrace = 2
  )
...
# Print all the confirmed variables with their coefficients
print(boruta.df)
```
...
```{r H20 AutoMl Variable Selection for Multinomial Classification}

# Identify predictors and response
y <- "RISK"
x <- setdiff(names(model.data.day), c("STRIKE", "STRIKECOUNT", "YEAR", "DAYOFWEEK", "HAIL", "DATE", "WSPD", "VISIB", "THUNDER", "DEWP", "MONTH", "RATIO", "RATIOP", "RATIOW", "WEEK", "FLIGHTCOUNT"))
...
```{r VIF}
# Select numeric variables for VIF
set.seed(123)
strikes.sample <-
  model.data.day[, c(
    "BIRDCOUNT",
...
```{r Boruta for Regression}
# Run Boruta on regression variables
var.boruta <-
  Boruta(
    STRIKECOUNT ~  BIRDCOUNT + FLIGHTCOUNT + MONTH + TEMP + VISIB + SLP + SEASON + MXSPD + PRCP,
    data = model.data.day,
    doTrace = 2
  )

# Plot Important variables on a box plot
lz <- lapply(1:ncol(var.boruta$ImpHistory), function(i)
  var.boruta$ImpHistory[is.finite(var.boruta$ImpHistory[, i]), i])
names(lz) <- colnames(var.boruta$ImpHistory)
Labels <- sort(sapply(lz, median))
...
glmStepAIC <-
  train(
    STRIKECOUNT ~  BIRDCOUNT + FLIGHTCOUNT + TEMP + VISIB + SLP + SEASON + MXSPD + PRCP,
    data = model.data.day,
    method = "glmStepAIC",
    allowParallel = TRUE
  )
...
# Get variable importance with coefficients
varImp(glmStepAIC)
